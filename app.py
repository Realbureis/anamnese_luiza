...
# 3. Mapeamento de Colunas (Interface Original preservada)
...
with tab1:
    ...
    with col2:
        ...
        if str(dados.get('gravida_sim')).lower() in ['true', '1.0', '1', 'sim']:
            st.warning("⚠️ Paciente Gestante")
...
with tab2:
    ...
    with c_canvas:
        if os.path.exists(caminho_img):
            ...
            # Com a versão 1.31.0 travada, o canvas funciona perfeitamente com Pillow
            canvas_result = st_canvas(
                ...
                width=400,
                drawing_mode="point",
                key=f"canvas_v_estavel_{paciente_selecionado}",
            )
        else:
            st.error(f"Arquivo {nome_img} não encontrado na raiz.")

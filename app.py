...
try:
    ...
    st.error("Erro ao conectar com a planilha. Verifique a conexão.")
    st.stop()
    ...
    if os.path.exists(caminho_img):
        with c_canvas:
            ...
            canvas_result = st_canvas(
                ...
                drawing_mode="point",
                key=f"canvas_reset_{paciente_selecionado}",
            )
        with c_form:
            if canvas_result and canvas_result.json_data and canvas_result.json_data["objects"]:
                ...
                if st.button("Salvar Medida"):
                    st.balloons()
            else: st.info("Clique na silhueta para marcar.")
    else: st.error(f"Imagem {nome_img} não encontrada.")
